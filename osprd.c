...
MODULE_AUTHOR("Breanna Nery & Garima Lunawat");
...
static void osprd_process_request(osprd_info_t *d, struct request *req)
{
	...
	unsigned long numBytes = (req->current_nr_sectors)*SECTOR_SIZE;
	// Process request
	switch(reqType)
	{
		case READ:
			memcpy((void*) req->buffer, (void*) d->data + sectorOffset, numBytes);
			break;
	...
	if (cmd == OSPRDIOCACQUIRE) {
		...
		switch(filp_writable)
		{
			case 0: /* READABLE */
				...
				break;
			default: /* WRITER */
				if(!delete_lock(current->pid, d->write_list))
				{
					osp_spin_unlock(&d->mutex);
					return -EINVAL;
				}
